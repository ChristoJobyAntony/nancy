...
def register():
    ...
    if form.validate_on_submit() :
        ...
        Query('users').delete().filter(date_of_join__gtHour = 4).filter(activated=False).execute().commit()
        # addUser(form.email.data,  form.password.data)
        hashed_password = sha224(form.password.data.encode()).hexdigest()
        if Query('users').get().filter(email=form.email.data).fetchone() :
            flash("An email had been already sent for activation please check you mail")
...
#Reset Password with sepecial token
